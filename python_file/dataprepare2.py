...
f = open('D:/Users/zjy/documents/bikeshare/2016_Q2.csv')
csv_f = csv.reader(f)
...
##count creat new table3
with con:
        cur = con.cursor()
        cur.execute("DROP TABLE IF EXISTS table3")
        cur.execute("CREATE TABLE table3(day INT, hour INT, value INT)")
        cur.execute("SELECT day, hour, SUM(value) FROM table2 GROUP BY day, hour")
        rows = cur.fetchall()
        for row in rows:
            insertStatement = """INSERT INTO table3(day, hour, value) VALUES ('%d','%d','%d')""" % (int(row[0]),int(row[1]),int(row[2]))
            cur.execute(insertStatement)
        con.commit()
...
with open("D:/Users/zjy/documents/bikeshare/heatmap_Q2.csv","wb") as csv_file:
